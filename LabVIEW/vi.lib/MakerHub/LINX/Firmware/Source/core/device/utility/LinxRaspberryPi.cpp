...
/*
{
	fprintf(stdout, "SpiWriteRead()\n");
	fprintf(stdout, "NumFrames = %d\n", numFrames);
	
	fprintf(stdout, "csChan = %d  :: csLL = %d\n", csChan, csLL);
	...
	struct spi_ioc_transfer transfer;
	//fprintf(stdout, "CS Wild\n");
	//Set CS As Output And Make Sure CS Starts Idle	
	DigitalWrite(csChan, (~csLL & 0x01) );
	//fprintf(stdout, "CS Idle\n");
	...
	for(int i=0; i<numFrames; i++)
	{
		...
		transfer.delay_usecs = 0;
		//transfer.speed_hz = SpiSetSpeeds[channel];
		transfer.speed_hz = 3900000;
		transfer.bits_per_word = 8;
		transfer.pad = 0;
	
		//CS Active
		DigitalWrite(csChan, csLL);			
		//fprintf(stdout, "CS Active\n");
		
		//Transfer Data
		//fprintf(stdout, "SpiHandles[channel] = %X\n", &SpiHandles[channel]);
		
		/*
		//This works
		int handle = open("/dev/spidev0.0", O_RDWR);
		int retVal = ioctl(handle, SPI_IOC_MESSAGE(1), &transfer);
		close(handle);
		////////
		...
		//{
			if(SpiHandles[channel] != NULL)
			{
				retVal = ioctl(SpiHandles[channel], SPI_IOC_MESSAGE(1), &transfer);
			}
			else
			{
				fprintf(stdout, "File handle is F'd\n");
			}
			
			//fprintf(stdout, "ioctl(%X, %X, %X)\n", &SpiHandles[channel], SPI_IOC_MESSAGE(1), &transfer);
			
			//for(int i=0; i<1; i++)
			//{
				//int monkey = 0;
				//monkey = ioctl(SpiHandles[channel], SPI_IOC_MESSAGE(1), &transfer);
			
				//CS Idle
				DigitalWrite(csChan, (~csLL & 0x01) );
				fprintf(stdout, "retVal =  %d   ::  errno = %d\n", retVal, errno);
				if (retVal < 0)
				{				
					DebugPrintln("SPI Fail - Failed To Transfer Data");
					return  LSPI_TRANSFER_FAIL;
