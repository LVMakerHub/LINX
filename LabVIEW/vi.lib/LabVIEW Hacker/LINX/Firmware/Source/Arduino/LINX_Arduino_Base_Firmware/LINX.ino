...
//#include <DNETcK.h>                                // ifdefs don't prevent these from being included in MPIDE, manually (un)comment as needed
...
//Include WIFI Headers If Necissary
  ...
  #include <DNETcK.h> 
  #include <DWIFIcK.h>                                // ifdefs don't prevent these from being included in MPIDE, manually (un)comment as needed
...
void linxI2CWrite(unsigned char* commandPacketBuffer, unsigned char* responsePacketBuffer)
{
  ...
  {
    Wire.write(commandPacketBuffer[i+8]);
    #ifdef DEBUG_ENABLED      
      Serial1.print("Sent ");
...
void linxI2CRead(unsigned char* commandPacketBuffer, unsigned char* responsePacketBuffer)
{
  ...
  if(Wire.available() >= commandPacketBuffer[8])
  {
    ...
    {
      responsePacketBuffer[i+4] = Wire.read();      
    }
